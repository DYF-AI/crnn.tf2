...
def get_image_path(dir_path):
    ...
    获取图片路径列表,及其标签列表
    '''
    if os.path.exists(config["work_space"]+'dataset/dataset.data'):
        with open(config["work_space"]+'dataset/dataset.data', 'rb') as f:
        ...
        with open(config["work_space"]+'dataset/dataset.data', 'wb') as f:
